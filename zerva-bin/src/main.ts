...
import { version as esbuildVersion } from 'esbuild/package.json'
...
export async function runMain(config: ZervaConf) {
  ...
  const ZERVA: ZervaBuildInfo = {
    ...
    build: config.build,
    buildEsbuildVersion: esbuildVersion,
    buildZervaVersion: config.version,
    buildNodeVersion: process.versions.node,
    buildESM: config.esm,
  }

  let banner = `/*  Generated by Zerva <https://github.com/holtwick/zerva> */\nglobalThis.ZERVA=${JSON.stringify(ZERVA)}\n`
